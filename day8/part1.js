  ...
  for (let i = 0; i < antennasPositions.length; i++) {
    for (let j = i + 1; j < antennasPositions.length; j++) {
      ...
      if (isSecondAntennaToTheRight) {
        antinode1RowIndex = secondAntennaRow + distanceHorizontal;
        antinode1ColIndex = secondAntennaCol + distanceVertical;
        antinode2RowIndex = firstAntennaRow - distanceHorizontal;
        antinode2ColIndex = firstAntennaCol - distanceVertical;
      } else {
        antinode1RowIndex = secondAntennaRow + distanceHorizontal;
        antinode1ColIndex = secondAntennaCol - distanceVertical;
        antinode2RowIndex = firstAntennaRow - distanceHorizontal;
